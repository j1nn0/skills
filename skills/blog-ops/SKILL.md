...
- **検査スクリプトの実行は、スキルを読んだことの代わりにならない。** `check_posts.py` と `check_style.py` が見るのは機械的に判定できる範囲だけで、各スキルの判断の大半はスクリプトに入っていない。スクリプトが 0 件で通っても、その段階は終わっていない。
- 裏取りのように、担当スキルの手順を読まずに自前の確認で済ませたものは、その工程を通したことにならない。自前の確認は下調べであって、`fact-check-ja` の判定と確認カードの代わりにはならない。
